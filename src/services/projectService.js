...
export async function getProjectStats(id) {
  ...
  if (!raw) return null;
  // Normalize: backend may return camelCase or PascalCase
  // ProjectStats.jsx expects PascalCase — normalize both
  const d = raw?.data || raw;
  return {
    // support both cases
    TotalTasks:           d?.TotalTasks           ?? d?.totalTasks           ?? 0,
    CompletedTasks:       d?.CompletedTasks        ?? d?.completedTasks       ?? d?.doneTasks      ?? 0,
    InProgressTasks:      d?.InProgressTasks       ?? d?.inProgressTasks      ?? d?.inProgress     ?? 0,
    OverdueTasks:         d?.OverdueTasks          ?? d?.overdueTasks         ?? 0,
    DueSoonTasks:         d?.DueSoonTasks          ?? d?.dueSoonTasks         ?? 0,
    CompletionPercentage: d?.CompletionPercentage  ?? d?.completionPercentage ?? d?.progressPercent ?? 0,
    TotalLoggedHours:     d?.TotalLoggedHours      ?? d?.totalLoggedHours     ?? (d?.totalTimeLogged ? d.totalTimeLogged / 60 : 0),
    TotalEstimatedHours:  d?.TotalEstimatedHours   ?? d?.totalEstimatedHours  ?? 0,
    StatusBreakdown:      d?.StatusBreakdown       ?? d?.statusBreakdown      ?? [],
    PriorityBreakdown:    d?.PriorityBreakdown     ?? d?.priorityBreakdown    ?? [],
    TopMembers:           d?.TopMembers            ?? d?.topMembers           ?? [],
    ActiveSprintStats:    d?.ActiveSprintStats     ?? d?.activeSprintStats    ?? null,
  };
}
