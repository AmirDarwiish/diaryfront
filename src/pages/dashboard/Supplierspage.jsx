...
import {
  Factory, Plus, Search, Eye, Pencil, Trash2,
...
══════════════════════════════════════════════════════════ */
function ConfirmModal({ supplier, onConfirm, onClose, loading }) {
  return (
    <div className="db-overlay" onClick={onClose}>
      <div className="db-modal" style={{ maxWidth: 400 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ fontSize: 14, color: 'var(--text-sec)', marginBottom: 20, lineHeight: 1.7 }}>
          ...
          <strong style={{ color: 'var(--text)' }}>{supplier.name}</strong>؟
          <br />
...
/* ══════════════════════════════════════════════════════════
   BALANCE MODAL  (GET /api/Suppliers/{id}/balance)
══════════════════════════════════════════════════════════ */
function BalanceModal({ supplier, onClose }) {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}/api/Suppliers/${supplier.id}/balance`,
          { headers: authHeaders() }
        )
  ...
  return (
    <div className="db-overlay" onClick={onClose}>
      <div className="db-modal" style={{ maxWidth: 380 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="db-modal__header">
          <span className="db-modal__title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Wallet size={18} color="var(--gold)" /> رصيد المورد
          </span>
          <button className="db-modal__close" onClick={onClose}>×</button>
        </div>
        ...
        <div style={{ textAlign: 'center', padding: '10px 0 20px' }}>
          ...
          {error && (
            <div className="db-error-box" style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '12px 0' }}>
          ...
          {balance !== null && !loading && (
            <div style={{
              ...
              <div style={{ fontSize: 11, marginTop: 6, color: 'var(--text-muted)' }}>
                {Number(balance) > 0 ? '⬆ مستحق على الشركة'
                  : Number(balance) < 0 ? '⬇ مستحق للشركة'
                  : 'الحساب متوازن'}
              </div>
            </div>
          )}
...
/* ══════════════════════════════════════════════════════════
   DETAIL MODAL  (GET /api/Suppliers/{id})
══════════════════════════════════════════════════════════ */
function DetailModal({ supplierId, onClose, onEdit }) {
  ...
  return (
    <div className="db-overlay" onClick={onClose}>
      <div className="db-modal" style={{ maxWidth: 480 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="db-modal__header">
          <span className="db-modal__title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Eye size={18} color="var(--gold)" /> تفاصيل المورد
          </span>
          <button className="db-modal__close" onClick={onClose}>×</button>
        </div>
        ...
        {data && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{
                  ...
                  fontSize: 11, fontWeight: 700, marginTop: 4, padding: '2px 8px',
                  borderRadius: 10,
                  background: data.isActive ? 'var(--green-bg)' : 'var(--red-bg)',
                  color: data.isActive ? 'var(--green)' : 'var(--red)',
                }}>
                  {data.isActive ? <CheckCircle size={11} /> : <X size={11} />}
            ...
            {[
              ...
              { icon: <Wallet size={15} />,      label: 'الرصيد الافتتاحي', value: `${fmt(data.openingBalance)} ج.م`, color: 'var(--blue)' },
              { icon: <TrendingDown size={15} />, label: 'الرصيد الحالي',   value: `${fmt(data.currentBalance)} ج.م`,
                color: (data.currentBalance ?? 0) >= 0 ? 'var(--green)' : 'var(--red)' },
...
function FormModal({ editData, onClose, onSuccess }) {
  ...
  const submit = async () => {
    ...
    try {
      const url    = isEdit
        ? `${API_BASE_URL}/api/Suppliers/${editData.id}`
        : `${API_BASE_URL}/api/Suppliers`
      const method = isEdit ? 'PUT' : 'POST'
      const body   = isEdit
        ? { name: form.name, phone: form.phone, address: form.address }
        : { name: form.name, phone: form.phone, address: form.address, OpeningBalance: Number(form.OpeningBalance) }

      const res  = await fetch(url, { method, headers: authHeaders(), body: JSON.stringify(body) })
      const json = await res.json()
      if (!res.ok) throw new Error(json.message || `HTTP ${res.status}`)
      onSuccess(isEdit ? 'تم تحديث بيانات المورد ✓' : 'تم إضافة المورد بنجاح ✓')
      onClose()
    } catch (e) {
      setError(e.message)
    } finally {
    ...
    <div className="db-overlay" onClick={onClose}>
      <div className="db-modal" style={{ maxWidth: 460 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            <label className="db-label">اسم المورد *</label>
            <input className="db-input" placeholder="مثال: مورد الألبان الطازجة"
              value={form.name} onChange={set('name')} />
          </div>
          <div>
            <label className="db-label">رقم الهاتف</label>
            <input className="db-input" placeholder="01xxxxxxxxx"
              value={form.phone} onChange={set('phone')} />
          </div>
          <div>
            <label className="db-label">العنوان</label>
            <input className="db-input" placeholder="المدينة، الشارع…"
              value={form.address} onChange={set('address')} />
          </div>
          {!isEdit && (
            <div>
              <label className="db-label">الرصيد الافتتاحي (ج.م)</label>
              <input type="number" className="db-input" placeholder="0.00" step="0.01"
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 4 }}>
            ...
            <button className="db-btn db-btn--gold" onClick={submit} disabled={loading}
              ...
              {loading
                ...
                : <>{isEdit ? <Pencil size={14} /> : <Plus size={15} />} {isEdit ? 'حفظ التعديلات' : 'إضافة المورد'}</>
              }
            </button>
          </div>
        </div>
...
/* ══════════════════════════════════════════════════════════
   TABLE
══════════════════════════════════════════════════════════ */
function SuppliersTable({ suppliers, onView, onEdit, onDelete, onBalance }) {
  if (suppliers.length === 0)
    return (
      <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--text-muted)' }}>
        <Factory size={40} style={{ opacity: 0.2, marginBottom: 12 }} />
        <div style={{ fontSize: 14, fontWeight: 700 }}>لا يوجد موردون</div>
        <div style={{ fontSize: 12, marginTop: 4 }}>ابدأ بإضافة أول مورد</div>
      </div>
    )
            ...
            <tr key={s.id}>
              ...
              <td>
                <span style={{
                  ...
                  color: (s.currentBalance ?? 0) >= 0 ? 'var(--green)' : 'var(--red)',
                }}>
...
function SuppliersContent() {
  ...
  // 'true' | 'false' — نبعت دايماً قيمة للـ API لأن الـ backend يتوقعها
  ...
  const load = useCallback(async (pg, s, active) => {
    ...
    setError(null)
    try {
      let url = `${API_BASE_URL}/api/Suppliers?page=${pg}&pageSize=${pageSize}&isActive=${active}`
  ...
  }, [page, search, isActive, load])

  /* ── Delete ───────────────────────────────────────────── */
  const handleDelete = async () => {
    setDeleteLoading(true)
    try {
      const res = await fetch(`${API_BASE_URL}/api/Suppliers/${deleteTarget.id}`, {
        method: 'DELETE', headers: authHeaders(),
      })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      setToast({ msg: 'تم حذف المورد بنجاح', type: 'ok' })
  ...
  const totalBalance = suppliers.reduce((s, x) => s + (x.currentBalance ?? 0), 0)
    ...
    <div className="db-page db-animate-in">
      ...
      {viewId && (
        <DetailModal
          ...
          onEdit={s => { setEditData(s); setViewId(null) }}
        />
      )}
      {(showCreate || editData) && (
        <FormModal
      ...
      {balanceTarget && (
        <BalanceModal supplier={balanceTarget} onClose={() => setBalanceTarget(null)} />
      )}
      ...
      <div style={{
        ...
        <div>
          ...
          <h1 style={{
            ...
            <Factory size={22} color="var(--purple)" /> الموردون
          </h1>
        </div>
        <button
          className="db-btn db-btn--gold"
      ...
      <div className="db-stats" style={{ gridTemplateColumns: 'repeat(3,1fr)', marginBottom: 20 }}>
        {[
          { label: 'إجمالي الموردين',    value: total.toLocaleString('ar-EG'), unit: 'مورد', color: 'var(--purple)', icon: <Users size={18} /> },
          { label: 'الموردون النشطون',   value: suppliers.filter(s => s.isActive !== false).length.toLocaleString('ar-EG'), unit: 'نشط', color: 'var(--green)', icon: <CheckCircle size={18} /> },
...
export default function SuppliersPage() {
  return (
    <DashboardLayout title="الموردون" breadcrumb="إدارة الموردين">
      <SuppliersContent />
    </DashboardLayout>
  )
}